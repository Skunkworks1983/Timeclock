...
public class SessionStore
{
            ...
            return DatabaseConnector.runQuery(query ->
               {
                   ...
                   if(delayUntilScheduled && start.isAfter(LocalTime.parse("00:00")) && start.isAfter(LocalTime.now()))
                   {
            ...
            DatabaseConnector.runQuery(query ->
               {
                   ...
                   long sessionStart = query.select(SESSIONS.START)
                                            ...
                                            .fetchAny()
                                            .get(SESSIONS.START);
                   double scheduledHours = scheduleStore.getScheduleOverlap(TimeUtil.getDateTime(sessionStart),
    ...
    {
        return DatabaseConnector.runQuery(query -> {
            Long result = query.selectFrom(SESSIONS).where(SESSIONS.START.ge(TimeUtil.getCurrentTimestamp())).fetchAny(SESSIONS.START);
            if(result != null)
            {
                return result;
            }
    ...
    public double calculateScheduledHours()
    {
        return DatabaseConnector.runQuery(query ->
                                              {
                                                  double total = 0;
                                                  for(float hours: query.selectFrom(SESSIONS).fetch(SESSIONS.SCHEDULEDHOURS))
                                                  {
                                                      total += hours;
                                                  }
                                                  return total;
